...
5. **Branching & PR Workflow:** When the user explicitly states a phase is done (e.g., "Phase B1 done"), you MUST create a new git branch for the modifications, commit all changes, and open a Pull Request to merge it into the main project branch (e.g., `gym-...`). Do not merge directly; always use a PR. **When creating a PR or providing a PR description to the user, you MUST always structure it using the repository's `.github/pull_request_template.md`.**
...
7. **Never Force Push:** NEVER use `git push -f` or `git push --force`. Always use standard commits and normal pushes to avoid rewriting git history or risking the work of other collaborators. If you need to fix a commit, make a new commit instead of amending and force-pushing. Never use `git commit --amend` or `git rebase` on any branch that has already been pushed to a remote repository; all corrections or PR feedback adjustments must be committed as brand-new, additive commits.
