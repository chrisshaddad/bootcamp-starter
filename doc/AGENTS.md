...
- Routes are protected by default (global `AuthGuard`). Use `@Public()` to opt out, `@Roles(...)` to restrict by role, `@AllowPending()` to let not-yet-onboarded (`PENDING`) users reach a handler, `@CurrentUser()` to get the calling user.
- **Two auth flows are supported and both are intentional — do not remove the password flow.** **Magic link (primary):** user enters email, receives a one-time link (15-min expiry, single-use, stored SHA-256-hashed) via Mailpit/SMTP, clicks it to get a session. **Password (secondary):** user authenticates with email + password directly (argon2-hashed). The `User.password` field is nullable, so accounts can exist without a password. Both flows issue the same cookie-based session (`bootcamp_starter_session`, `HttpOnly`, 30-day TTL).
- **Account status gates authentication.** `SUSPENDED`/`INACTIVE` can never authenticate. `PENDING` (invited staff) may verify a magic link and reach the set-password step, but is denied every other protected route unless the handler opts in with `@AllowPending()`. Setting a password flips `PENDING` → `ACTIVE`, completing onboarding.
- **Auth responses are non-enumerable:** magic-link requests, password login, and signup return generic results (and run in comparable time) so callers can't probe which emails exist. Preserve this when touching the auth flows.
