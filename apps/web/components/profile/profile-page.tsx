...
//
// latitude/longitude stay in the schema even when the location UI is hidden:
// they are seeded from the stored profile and submitted unchanged, so a user
// without the location fields never wipes their coordinates.
...
function ProfileForm({
  ...
  const onSubmit = async (data: ProfileFormValues) => {
    try {
      // When the location UI is hidden the seeded lat/lng ride along untouched,
      // so editing other fields never clears the stored coordinates.
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      >
        ...
        <CardContent className="space-y-4">
          ...
          {/* Location is opt-in — pharmacy staff manage a personal profile with
              no map/coordinates. The values still submit unchanged (see
              onSubmit) when this block isn't rendered. */}
          {includeLocation ? (
            <>
              <div className="space-y-1.5">
...
/**
 * The signed-in user's own profile (GET/PATCH /profile). Shared by the super
 * admin console and the pharmacy panel. `includeLocation` adds the map +
 * lat/lng fields (admin only); pharmacy users manage a location-free profile.
 */
  ...
  return (
    <div className="space-y-6">
      ...
      {error ? (
        ...
        <ProfileForm profile={profile} includeLocation={includeLocation} />
      )}
    </div>
  );
}
