...
// Shared audit console UI. The super-admin console (`/admin/audit`, full
// platform feed) and the pharmacy console (`/pharmacy/audit`, scoped to one
// pharmacy's staff) render the same component — only the data source and the
// heading copy differ, so the two never drift. All tenant scoping happens
// server-side; this component just displays what the hook returns.
...
export function AuditConsole({
  ...
  // Filter the fetched window in the client — category is derived from the event
  // key, and search spans both the human text and the raw identifiers so
  // engineers can still find a specific event.
  const rows = useMemo(() => {
    if (!logs) return logs;
          ...
          <div className="flex flex-1 flex-col items-center justify-center py-16">
            ...
            <p className="max-w-md text-center text-sm text-gray-500">
              {logs && logs.length > 0
                ...
                : 'Activity will appear here as actions are recorded.'}
            </p>
          </div>
        ) : (
