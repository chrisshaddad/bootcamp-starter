...
import type { StudentOrganizationsResponse } from '@repo/contracts';

import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Skeleton } from '@/components/ui/skeleton';
import { fetcher } from '@/lib/api';

type StudentOrganizationCard =
  StudentOrganizationsResponse['organizations'][number];
...
export default function StudentsPage() {
  const [organizations, setOrganizations] = useState<StudentOrganizationCard[]>(
    [],
  );
  ...
  useEffect(() => {
    async function loadOrganizations() {
      ...
      try {
        const data = await fetcher<StudentOrganizationsResponse>(
          ...
          <CardContent className="p-6">
            <div className="flex min-h-28 flex-col items-center justify-center text-center">
              ...
              <p className="mt-1 text-xs text-[#7b8598]">
                No organizations with students are currently available.
              </p>
            </div>
          </CardContent>
        </Card>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-xl font-bold text-[#17223b]">
                          {formatNumber(organization.studentCount)}
                        </p>
                      </div>
                    </div>
