...
import { Clock, UserPlus, FileEdit, Trash2, Calendar, Fingerprint, Dumbbell, Shield, AlertCircle } from 'lucide-react';
...
function getActionDetails(action: string, entityType: string) {
  ...
  if (normalizedAction.includes('CREATE')) {
    return { icon: UserPlus, colorClass: 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' };
  }
  if (normalizedAction.includes('UPDATE')) {
    return { icon: FileEdit, colorClass: 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' };
  }
  if (normalizedAction.includes('DELETE') || normalizedAction.includes('CANCEL')) {
    return { icon: Trash2, colorClass: 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' };
  }
  if (entityType === 'CHECK_IN') {
    return { icon: Fingerprint, colorClass: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400' };
  }
  if (entityType === 'SESSION' || entityType === 'BOOKING') {
    return { icon: Calendar, colorClass: 'bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400' };
  }
  if (entityType === 'INSTRUCTOR') {
    return { icon: Dumbbell, colorClass: 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400' };
  }
  
  return { icon: Shield, colorClass: 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400' };
}
...
function formatActionText(log: AuditLogResponse) {
  ...
  return (
    <span>
      ...
      <span className="text-muted-foreground">{actionLower} {entityTypeLower}</span>{' '}
      {log.entityName && <span className="font-semibold text-foreground">{log.entityName}</span>}
    </span>
  );
}
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-xl font-semibold mb-2">Failed to load activity log</h2>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Track changes and activities across your gym</p>
      </div>
      ...
      {!auditLogs?.length ? (
        ...
        <div className="space-y-4 animate-stagger relative before:absolute before:inset-0 before:ml-8 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-border before:to-transparent">
          {auditLogs.map((log) => {
            const { icon: ActionIcon, colorClass } = getActionDetails(log.action, log.entityType);
      ...
      {totalPages && totalPages > 1 && (
        <div className="flex items-center justify-between pt-6 border-t border-border">
          <p className="text-sm text-muted-foreground">
            Showing {(currentPage - 1) * 20 + 1} to {Math.min(currentPage * 20, total || 0)} of {total} entries
          </p>
          <div className="flex gap-2">
            <Button
              variant="outline"
