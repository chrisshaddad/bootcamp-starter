...
const publicRoutes = ['/login', '/signup', '/auth/magic-link/verify'];
...
// 1. CHANGED: Renamed "proxy" to "middleware" so Next.js runs it, and made it async
      ...
      try {
        ...
        if (apiRes.ok) {
          ...
          if (sessionId) {
            ...
            const response = NextResponse.redirect(new URL(DEFAULT_AUTHENTICATED_ROUTE, request.url));
