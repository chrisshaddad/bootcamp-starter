...
2. Log in as `admin@medilink.local` (Hoda Faour) — confirm `/institutions` shows exactly 3 rows (Central Perk, Cedar Heights Medical Group, Bellerive Health Clinic) — the reserved platform institution is deliberately excluded from that list by design, so 3 is correct, not 4.
